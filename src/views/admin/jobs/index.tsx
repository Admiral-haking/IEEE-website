...
export default function JobsAdminView() {
  ...
  const [{ data }, refetch] = useAxios({ url: '/api/jobs', params: { page: page + 1, pageSize: rowsPerPage, locale } });
  ...
  const onDelete = async (row: Row) => { await deleteReq({ url: `/api/jobs/${row.id}` }); await refetch(); };
