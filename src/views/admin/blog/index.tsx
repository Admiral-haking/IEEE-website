...
export default function BlogAdminView() {
  ...
  const [{ data }, refetch] = useAxios({ url: '/api/blog', params: { q: query, page: page + 1, pageSize: rowsPerPage, locale } });
  ...
  const onDelete = async (row: Row) => { await deleteReq({ url: `/api/blog/${row.id}` }); await refetch(); };
